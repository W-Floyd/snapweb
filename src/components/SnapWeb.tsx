...
// Autoplay policy is queried once — it cannot change without a page reload.
function getAutoplayPolicy(): string {
  if ("getAutoplayPolicy" in navigator) {
    try {
      return (navigator as any).getAutoplayPolicy("mediaelement");
    } catch {
      // fall through
    }
  }
...
export default function SnapWeb() {
  ...
  const snapControlRef = useRef<SnapControl>(null!);
  if (!snapControlRef.current) snapControlRef.current = new SnapControl();

  // Set AudioSession type to playback to enable background audio on iOS.
  // Runs once — setting it on every render is wasteful.
  useEffect(() => {
    if ("audioSession" in navigator) {
      ((navigator as any).audioSession.type as string) = "playback";
    }
  }, []);
